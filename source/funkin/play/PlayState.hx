...
class PlayState extends MusicBeatSubState
{
  ...
  **/
  ...
  function initHealthBar():Void
  {
    ...
    scoreText = new FlxText(healthBarBG.x + healthBarBG.width - 150, healthBarBG.y + 30, 0, '', 20);
    scoreText.setFormat(Paths.font('ssp.ttf'), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
  ...
  function onNoteMiss(note:NoteSprite, playSound:Bool = false, healthChange:Float):Void
  {
    ...
    if (Highscore.tallies.combo != 0) if (Highscore.tallies.combo >= 0) comboPopUps.displayCombo(0);
  ...
  function popUpScore(daRating:String, ?combo:Int):Void
  {
    ...
    if (combo >= 0 || combo == 0) comboPopUps.displayCombo(combo);
